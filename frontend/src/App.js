...
// ==========================================
// 2. HOME PAGE (Massive Product Grid)
// ==========================================
const Home = ({ products, addToCart }) => (
  <div style={{ minHeight: '80vh' }}>
    ...
    <div style={{ padding: '0 40px 40px 40px' }}>
      ...
      {/* If grid is empty, show a loading/empty state */}
      {products.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '20px', marginTop: '20px' }}>
          {products.map((product, index) => (
            <div key={index} style={{ backgroundColor: 'white', padding: '15px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column' }}>
              <img src={product.image || 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=500&q=80'} alt={product.name} style={{ width: '100%', height: '180px', objectFit: 'cover', borderRadius: '4px', marginBottom: '15px' }} />
...
const Dashboard = () => {
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/products', newProduct, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '1200px', margin: '0 auto', minHeight: '70vh' }}>
      ...
      {/* TAB 2: CUSTOMER MESSAGES (UI Mockup) */}
      {activeTab === 'messages' && (
  ...
  <footer style={{ backgroundColor: '#2e2e54', color: '#fff', padding: '40px 20px', marginTop: 'auto' }}>
    <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '20px' }}>
      <div style={{ flex: 1, minWidth: '200px' }}>
        <h3 style={{ color: '#f85606', marginBottom: '15px' }}>Customer Care</h3>
        <p style={footerLink}>Help Center</p>
        <p style={footerLink}>How to Buy</p>
        <p style={footerLink}>Returns & Refunds</p>
        <p style={footerLink}>Contact Us</p>
      </div>
      <div style={{ flex: 1, minWidth: '200px' }}>
        <h3 style={{ color: '#f85606', marginBottom: '15px' }}>Earn With SuperShop</h3>
        <p style={footerLink}>Sell on SuperShop</p>
        <p style={footerLink}>Code of Conduct</p>
        <p style={footerLink}>Join the Affiliate Program</p>
      </div>
      <div style={{ flex: 1, minWidth: '200px' }}>
        <h3 style={{ color: '#f85606', marginBottom: '15px' }}>Online SuperShop</h3>
        <p style={footerLink}>About Us</p>
        <p style={footerLink}>Digital Payments</p>
        <p style={footerLink}>Privacy Policy</p>
      </div>
      <div style={{ flex: 1, minWidth: '200px' }}>
        <h3 style={{ color: '#f85606', marginBottom: '15px' }}>Payment Methods</h3>
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          <span style={paymentBadge}>Bkash</span>
          <span style={paymentBadge}>Nagad</span>
          <span style={paymentBadge}>Visa</span>
          <span style={paymentBadge}>MasterCard</span>
        </div>
      </div>
    </div>
    <div style={{ textAlign: 'center', marginTop: '40px', paddingTop: '20px', borderTop: '1px solid #444', fontSize: '14px', color: '#aaa' }}>
      © 2026 Online SuperShop. All Rights Reserved. Practicum Project.
    </div>
  </footer>
);
...
function AppContent() {
  ...
  // MASSIVE FALLBACK DATASET SO THE STORE NEVER LOOKS EMPTY!
  const backupProducts = [
    ...
    { id: 109, name: 'Logitech Wireless Mouse', price: 4500, category: 'Computers', image: 'https://images.unsplash.com/photo-1593640408182-31c70c8268f5?w=500&q=80' },
    { id: 110, name: 'Ergonomic Office Chair', price: 12000, category: 'Furniture', image: 'https://images.unsplash.com/photo-1505843490538-5133c6c7d0e1?w=500&q=80' },
    { id: 111, name: 'Vitamin C Face Serum', price: 1500, category: 'Beauty', image: 'https://images.unsplash.com/photo-1620916566398-39f1143ab7be?w=500&q=80' },
    { id: 112, name: 'Travel Duffle Bag', price: 2800, category: 'Travel', image: 'https://images.unsplash.com/photo-1553062407-98eeb64c6a62?w=500&q=80' },
  ];

  useEffect(() => {
    // Try to get DB products, but immediately fall back to the massive list if DB is empty
    axios.get('http://localhost:5000/products')
      .then(response => {
        if (response.data && response.data.length > 0) {
          // Combine DB products with backup products to ensure the store ALWAYS looks full
    ...
    <div style={{ fontFamily: 'Arial, sans-serif', backgroundColor: '#f4f4f4', minHeight: '100vh', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      ...
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 40px', backgroundColor: '#f85606', color: 'white', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
          ...
          {/* STRICT ROLE CHECK: Only Sellers see the Dashboard */}
          {!role ? (
            <>
            ...
            <>
              {(role === 'seller' || role === 'admin') && (
                <Link to="/dashboard" style={navButton}>Vendor Dashboard</Link>
              )}
      ...
      {/* FOOTER IS ALWAYS AT THE BOTTOM NOW */}
      <Footer />
